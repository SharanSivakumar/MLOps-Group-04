...
def test_data_module_initialization():
    ...
    data_module = ECGDataModule(data_dir="data/raw", processed_dir="data/processed", batch_size=32, num_workers=0)
...
def test_data_module_with_different_batch_sizes():
    ...
    for batch_size in [8, 16, 32, 64]:
        data_module = ECGDataModule(
            data_dir="data/raw", processed_dir="data/processed", batch_size=batch_size, num_workers=0
        )
...
def test_dataloader_shapes():
    ...
    data_module = ECGDataModule(data_dir="data/raw", processed_dir="data/processed", batch_size=16, num_workers=0)
    data_module.setup()
...
def test_val_test_loaders_dont_shuffle():
    ...
    assert isinstance(val_loader.sampler, SequentialSampler), (
        "Validation loader should use SequentialSampler (shuffle=False)"
    )
    assert isinstance(test_loader.sampler, SequentialSampler), (
        "Test loader should use SequentialSampler (shuffle=False)"
    )
